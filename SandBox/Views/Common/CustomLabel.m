...
@implementation NSString (CustomLabelDrawing)

- (void)CustomLabel_pushSizingContext
{
    
#if !__IPHONE_TARGET_IOS7
...
- (void)CustomLabel_popSizingContext
{
...
- (BOOL)CustomLabel_isPunctuation
{
...
- (NSArray *)CustomLabel_characters
{
...
- (NSArray *)CustomLabel_linesWithFont:(UIFont *)font
                          ...
                          allowOrphans:(BOOL)allowOrphans
{
    ...
    if (lineBreakMode == NSLineBreakByCharWrapping)
    {
        ...
        NSArray *characters = [self CustomLabel_characters];
            ...
            for (NSUInteger i = index; i < [words count]; i++)
            {
                ...
                else if (lineWidth > size.width && line)
                {
                    ...
                    if (!allowOrphans && i > 0 &&
                        ...
                        ![words[i - 1] CustomLabel_isPunctuation])
                    {
                        //force line break
...
- (CGSize)CustomLabel_sizeWithFont:(UIFont *)font
{
...
- (CGSize)CustomLabel_drawAtPoint:(CGPoint)point withFont:(UIFont *)font color:(UIColor *)color
{
                  ...
                  includesEllipsis:(BOOL *)includesEllipsis
{
    ...
    [self CustomLabel_pushSizingContext];
    ...
    while (true)
    {
        ...
        for (i = 0; i < charCount; i++)
        {
            //get character width
            NSString *character = characters[i];
            CGFloat charWidth = [character CustomLabel_sizeWithFont:subFont].width;
            if (i < charCount - 1)
            {
        ...
        if (floor(x) <= ceil(width))
        {
            ...
            [self CustomLabel_popSizingContext];
        ...
        else if (subFont.pointSize <= minFontSize)
        {
            ...
            else
            {
                ...
                CGFloat ellipsisWidth = [@"…" CustomLabel_sizeWithFont:subFont].width;
...
- (CGSize)sizeWithFont:(UIFont *)font
      ...
      characterSpacing:(CGFloat)characterSpacing
          kerningTable:(NSDictionary *)kerningTable
{
    if (characterSpacing || [kerningTable count] || __IPHONE_TARGET_IOS7)
    {
        CGSize size = [self CustomLabel_sizeWithFont:font
...
- (CGSize)CustomLabel_drawAtPoint:(CGPoint)point
                         forWidth:(CGFloat)width
                            ...
                            color:(UIColor *)color
{
    ...
    BOOL usingMask = !color && __IPHONE_TARGET_IOS7;
    
    BOOL drawCharByChar = characterSpacing || [kerningTable count];
    if (drawCharByChar)
    {
        size = [self CustomLabel_sizeWithFont:font
                                  minFontSize:minFontSize
    ...
    if (fontSize < font.pointSize && (usingMask || drawCharByChar || __IPHONE_TARGET_IOS7))
    {
    ...
    if (drawCharByChar)
    {
        ...
        for (NSUInteger i = 0; i < charactersFitted; i++)
        {
            ...
            CGFloat charWidth = [character CustomLabel_drawAtPoint:CGPointMake(x, 0) withFont:subFont color:color].width;
            x += charWidth + ([kerningTable[character] floatValue] + characterSpacing) * subFont.pointSize;
        }
        if (includesEllipsis)
        {
            [@"…" CustomLabel_drawAtPoint:CGPointMake(x, 0) withFont:subFont color:color];
        }
    }
    else
    {
...
- (CGSize)drawAtPoint:(CGPoint)point
   ...
   baselineAdjustment:(UIBaselineAdjustment)baselineAdjustment
     characterSpacing:(CGFloat)characterSpacing
         kerningTable:(NSDictionary *)kerningTable
{
    return [self CustomLabel_drawAtPoint:point
                                forWidth:width
                                withFont:font
                             minFontSize:minFontSize
                          actualFontSize:actualFontSize
...
- (CGSize)sizeWithFont:(UIFont *)font
     constrainedToSize:(CGSize)size
      ...
      characterSpacing:(CGFloat)characterSpacing
          ...
          allowOrphans:(BOOL)allowOrphans
{
    if (lineSpacing || characterSpacing || [kerningTable count] || !allowOrphans || __IPHONE_TARGET_IOS7)
    {
        NSArray *lines = [self CustomLabel_linesWithFont:font
...
- (CGSize)CustomLabel_drawInRect:(CGRect)rect
                        withFont:(UIFont *)font
                   lineBreakMode:(NSLineBreakMode)lineBreakMode
                           ...
                           color:(UIColor *)color
{
    if (lineSpacing || characterSpacing || [kerningTable count] || !allowOrphans || __IPHONE_TARGET_IOS7)
    {
        ...
        for (NSString *line in lines)
        {
            ...
            [line CustomLabel_drawAtPoint:offset
                                 forWidth:CGRectGetWidth(rect)
                                 withFont:font
                              minFontSize:font.pointSize
                           actualFontSize:NULL
...
- (CGSize)drawInRect:(CGRect)rect
    ...
    characterSpacing:(CGFloat)characterSpacing
        ...
        allowOrphans:(BOOL)allowOrphans
{
    return [self CustomLabel_drawInRect:rect
                               withFont:font
                          lineBreakMode:lineBreakMode
                              alignment:alignment
                            lineSpacing:lineSpacing
...
- (CGSize)CustomLabel_sizeThatFits:(CGSize)size actualFontSize:(CGFloat *)actualFontSize
{
...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    size = [self CustomLabel_sizeThatFits:size actualFontSize:NULL];
    size.width += (self.textInsets.left + self.textInsets.right);
    size.height += (self.textInsets.top + self.textInsets.bottom);
...
- (void)CustomLabel_getComponents:(CGFloat *)rgba forColor:(CGColorRef)color
{
...
- (UIColor *)CustomLabel_color:(CGColorRef)a blendedWithColor:(CGColorRef)b
{
    CGFloat aRGBA[4];
    [self CustomLabel_getComponents:aRGBA forColor:a];
    CGFloat bRGBA[4];
    [self CustomLabel_getComponents:bRGBA forColor:b];
...
- (void)CustomLabel_drawTextInRect:(CGRect)rect withFont:(UIFont *)font color:(UIColor *)color
{
    rect.origin.y += font.pointSize * self.baselineOffset;
    if (self.numberOfLines == 1)
    {
        [self.text CustomLabel_drawAtPoint:rect.origin
    ...
    {
        [self.text CustomLabel_drawInRect:rect
                                 withFont:font
                            lineBreakMode:self.lineBreakMode
...
- (void)drawRect:(CGRect)rect
{
    ...
    textRect.size = [self CustomLabel_sizeThatFits:contentRect.size actualFontSize:&fontSize];
    ...
    if (needsMask)
    {
        ...
        [self CustomLabel_drawTextInRect:textRect withFont:font color:[UIColor blackColor]];
        CGContextRestoreGState(context);
    ...
    if (needsMask)
    {
        ...
        if (hasGradient)
        {
            ...
            {
                UIColor *blended = [self CustomLabel_color:color.CGColor blendedWithColor:textColor.CGColor];
    ...
    {
        //just draw the text
        [self CustomLabel_drawTextInRect:textRect withFont:font color:textColor];
    }
